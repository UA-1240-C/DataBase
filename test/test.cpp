...
const static std::string CONNECTION_STRING = "dbname=<> user=postgres password=<> hostaddr=127.0.0.1 port=5432";
...
TEST_F(DatabaseFixture, Insert_Email_With_Multiple_Recepients_Test)
{
    EXPECT_NO_THROW(m_database->InsertEmail("user1", {"user2", "user3"}, "subjectsubject1", "body1"));
    EXPECT_EQ(2, m_database->RetrieveEmails("user2").size());
    EXPECT_EQ(2, m_database->RetrieveEmails("user3").size());

    EXPECT_EQ(1, m_database->RetrieveEmailContentInfo("body1").size());
}

TEST_F(DatabaseFixture, Insert_Email_With_Multiple_Unexisting_Recepients_Test)
{
    EXPECT_ANY_THROW(m_database->InsertEmail("user1", {"user4", "user54"}, "subjectsubject1", "body1"));
}

TEST_F(DatabaseFixture, Insert_Mail_Content_Test)
{
    EXPECT_NO_THROW(m_database->InsertEmailContent("new body"));
    EXPECT_EQ(1, m_database->RetrieveEmailContentInfo("new body").size());
}

TEST_F(DatabaseFixture, Insert_Existing_Mail_Content_Test)
{
    EXPECT_NO_THROW(m_database->InsertEmailContent("looooong body1"));
    EXPECT_EQ(2, m_database->RetrieveEmailContentInfo("looooong body1").size());
}
